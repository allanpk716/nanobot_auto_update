...
1. **downloadWithProgress 是否替代现有 download 方法？**
   - What we know: 现有 download 方法被 Update 方法调用两次（checksums + zip），CheckLatest 用 httpClient.Get 而非 download。
   - What's unclear: 是否需要保持 download 方法不变，新增 downloadWithProgress？还是统一替换？
   - Recommendation: 统一替换 download 为 downloadWithProgress，仅在 ZIP 下载时更新进度（通过 stage 参数控制），checksums 下载不更新百分比。

2. **ProgressState 生命周期与 SelfUpdateStatus 的关系？**
   ...
   - What's unclear: 两者是独立追踪还是合并为一个？
   - Recommendation: ProgressState 在 selfupdate.Updater 中追踪下载进度，SelfUpdateStatus 在 api.SelfUpdateHandler 中追踪 HTTP API 层面的状态。两者独立，前端同时看到两个维度。
