...
- [ ] **CAPT-04**: 系统在 nanobot 进程启动时自动开始捕获输出
- [ ] **CAPT-05**: 系统在 nanobot 进程停止时自动停止捕获输出

### Log Buffering

- [ ] **BUFF-01**: 系统为每个 nanobot 实例维护独立的环形缓冲区 (Circular Buffer)
- [ ] **BUFF-02**: 系统限制每个实例的缓冲区大小为 5000 行日志
- [ ] **BUFF-03**: 系统使用线程安全的环形缓冲区实现，支持并发读写
- [ ] **BUFF-04**: 系统在缓冲区满时自动覆盖最旧的日志 (FIFO)
- [ ] **BUFF-05**: 系统为每条日志保留时间戳、来源 (stdout/stderr) 和内容

### SSE Streaming

- [ ] **SSE-01**: 系统提供 `/api/v1/logs/:instance/stream` SSE 端点用于实时日志流
- [ ] **SSE-02**: 系统使用 Server-Sent Events 协议推送日志到客户端
- [ ] **SSE-03**: 系统每 30 秒发送 SSE 心跳注释防止连接超时
- [ ] **SSE-04**: 系统检测客户端断开连接并停止发送事件
- [ ] **SSE-05**: 系统在客户端连接时发送缓冲区中的历史日志
- [ ] **SSE-06**: 系统将 stdout 和 stderr 分别标记为不同事件类型 (便于客户端区分)
- [ ] **SSE-07**: 系统设置 HTTP WriteTimeout 为 0 以支持长连接

### Web UI

- [ ] **UI-01**: 系统提供 `/logs/:instance` HTML 页面查看日志
- [ ] **UI-02**: 系统在 Web 页面自动滚动到最新日志 (类似 tail -f)
- [ ] **UI-03**: 用户可以通过按钮暂停和恢复自动滚动
- [ ] **UI-04**: 系统使用不同颜色区分 stdout 和 stderr 输出
- [ ] **UI-05**: 系统在页面上显示 SSE 连接状态 (连接中/已连接/已断开)
- [ ] **UI-06**: 系统将静态 HTML/CSS/JS 文件嵌入到 Go 二进制中 (单文件部署)
- [ ] **UI-07**: 系统提供实例选择下拉菜单，用于切换查看不同实例的日志

### Instance Management Integration

- [ ] **INST-01**: 系统将 LogBuffer 集成到 InstanceLifecycle 结构中
- [ ] **INST-02**: 系统在 InstanceManager 中管理所有实例的 LogBuffer
- [ ] **INST-03**: 系统在实例启动时创建对应的 LogBuffer
- [ ] **INST-04**: 系统在实例停止时保留 LogBuffer (可查看历史日志)
- [ ] **INST-05**: 系统在实例重启时清空 LogBuffer (重新开始缓冲)
...
- [ ] **ERR-04**: 系统在请求不存在的实例日志时返回 HTTP 404 Not Found
